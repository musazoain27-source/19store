...
export default async function HomePage({ searchParams }) {
  ...
  return (
    <div>
      {/* Hero */}
      <section className="relative h-[70vh] min-h-[420px] flex items-center overflow-hidden bg-brand">
        <Image
      ...
      {/* Promo banners */}
      <section className="max-w-7xl mx-auto px-4 md:px-6 -mt-10 relative z-10 grid grid-cols-1 md:grid-cols-2 gap-5">
        <Link
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-6 py-16">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
          {categories.map((cat) => (
            <Link key={cat} href={`/category/${encodeURIComponent(cat)}`} className="group card-hover rounded-2xl overflow-hidden relative aspect-square">
              ...
              <div className="absolute inset-0 bg-neutral-900/30 group-hover:bg-neutral-900/40 transition-colors flex items-center justify-center">
                <span className="text-white font-semibold text-lg tracking-wide">{cat}</span>
              </div>
            </Link>
          ))}
        </div>
