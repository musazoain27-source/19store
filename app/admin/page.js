    ...
    <div className="animate-fadeIn">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
        {stats.map(({ label, value, icon: Icon }) => (
          <div key={label} className="bg-white border border-black/10 rounded-2xl p-5">
            <Icon size={20} className="text-brand-blue mb-3" />
        ...
        <div className="bg-white border border-black/10 rounded-2xl p-6">
          ...
          <div className="space-y-3">
            {orders.slice(0, 5).map((o) => (
              <Link key={o.id} href="/admin/orders" className="flex items-center justify-between text-sm hover:text-brand-blue transition-colors">
        ...
        <div className="bg-white border border-black/10 rounded-2xl p-6">
          ...
          <div className="space-y-3">
            {lowStock.slice(0, 5).map((p) => (
              <Link key={p.id} href={`/admin/products/${p.id}/edit`} className="flex items-center justify-between text-sm hover:text-brand-blue transition-colors">
                <span className="font-medium">{p.title}</span>
                <span className="text-red-500">{Object.values(p.sizes || {}).reduce((a, b) => a + b, 0)} left</span>
              </Link>
            ))}
            {lowStock.length === 0 && <p className="text-sm text-black/40">All products are well stocked.</p>}
          </div>
        </div>
      </div>
    </div>
